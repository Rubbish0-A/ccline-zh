...
### 变更（context 配色统一为「双刻度取更危险者」，真机对照拍板）
- v0.3.0 的配色规则双轨不一致：弹性超窗分支按绝对量阈值、普通路径按相对
  剩余阈值，同一物理状态（41% of 1M）在裸 model id 与显式 `[1m]` 两种模式
  下一红一绿，切 `[1m]` 后颜色突变被误读为 bug。
- 统一为**两条独立风险刻度取更危险者**，两条路径同一规则：
  - **相对剩余阈值**（窗口空间，`thresholds`）：200K 窗口快用满时变红；
  - **绝对量警戒线**（模型质量 context rot，默认 300K 黄 / 400K 红）：1M
    窗口下 400K 即红——纯相对刻度要到 800K 才红，对大窗口警示太晚。
  - 任一刻度亮黄/红即显示该色；跨模式天然同色。
- 绝对警戒线可调（`tokensWarn` / `tokensDanger`）、可关（`"tokensWarn": 0`
  退回纯相对刻度）。

### 实测备注
- `claude-fable-5[1m]` 经第三方中转三层探针实测可用（CC 把 `[1m]` 转为
  `context-1m` beta header，model 字段仍是裸 id，不触发中转模型白名单）。
  切 `[1m]` 后 CC 报真 1M 窗口，百分比原生正确，本插件弹性分支自动退出，
  CC 原生 UI 的 "100% context used" 假警报同步消失。
